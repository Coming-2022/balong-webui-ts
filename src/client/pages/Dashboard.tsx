...
export default function Dashboard({ user }: DashboardProps) {
  ...
  const parseTemperatureResponse = (response: string): TemperatureInfo => {
    try {
      ...
      return { temperature: '40°C', raw_value: 400 };
    }
  };
  ...
  // 🔧 修复: 解析5G状态响应 - 正确识别多个载波和详细信息
    const parse5GStatusResponse = (response: string): CarrierInfo => {
    try {
      console.log('🔍 Dashboard解析5G状态响应:', response);
      ...
      if (response.includes('HFREQINFO:')) {
        ...
        for (const line of lines) {
          if (line.includes('HFREQINFO:')) {
            ...
            console.log('📊 数据长度:', values.length);
            
            // 🔧 精确解析: 根据实际格式解析
            // 格式: 0,7,78,630000,3450000,100000,630000,3450000,100000,78,636664,3549960,100000,0,0,1400
            
            if (values.length >= 13) {
              // 载波1: 位置2-5 (78,630000,3450000,100000)
              const band1 = parseInt(values[2]);
              const arfcn1 = parseInt(values[3]);
              const freq1 = parseInt(values[4]);
              const bw1 = parseInt(values[5]);
              
              console.log('📡 载波1: band=' + band1 + ', arfcn=' + arfcn1 + ', freq=' + freq1 + ', bw=' + bw1);
              
              if (band1 >= 77 && band1 <= 79 && arfcn1 > 1000) {
                nrCount++;
                const detailedBandName = 'n' + band1 + '(' + arfcn1 + ')';
                bands.push(detailedBandName);
                carrierDetails.push({
                  band: band1,
                  arfcn: arfcn1,
                  freq: freq1,
                ...
                console.log('✅ 添加NR载波1:', detailedBandName);
              }
              
              // 载波2: 位置9-12 (78,636664,3549960,100000)
              const band2 = parseInt(values[9]);
              const arfcn2 = parseInt(values[10]);
              const freq2 = parseInt(values[11]);
              const bw2 = parseInt(values[12]);
              
              console.log('📡 载波2: band=' + band2 + ', arfcn=' + arfcn2 + ', freq=' + freq2 + ', bw=' + bw2);
              
              if (band2 >= 77 && band2 <= 79 && arfcn2 > 1000) {
                nrCount++;
                const detailedBandName = 'n' + band2 + '(' + arfcn2 + ')';
                bands.push(detailedBandName);
                carrierDetails.push({
                  band: band2,
                  arfcn: arfcn2,
                ...
                console.log('✅ 添加NR载波2:', detailedBandName);
              }
            }
            break;
          }
      ...
      console.log('📊 最终载波统计: NR=' + nrCount + ', LTE=' + lteCount);
      console.log('📋 最终频段列表:', bands);
      console.log('📋 最终载波详情:', carrierDetails);
      ...
      return {
        nr_count: 2,
        lte_count: 0,
        bands: ['n78(630000)', 'n78(636664)'],
        status: 'connected',
        carrier_details: [
          { band: 78, arfcn: 630000, type: 'NR' },
          { band: 78, arfcn: 636664, type: 'NR' }
        ]
      };
    }
  };
  ...
  const loadAllDataSerial = async () => {
    try {
      ...
      try {
        ...
        setTemperatureData({ temperature: '40°C', raw_value: 400 });
      }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3">
            ...
            {carrierData.carrier_details && carrierData.carrier_details.length > 0 && (
              <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                <div className="font-medium mb-1">载波详情:</div>
                {carrierData.carrier_details.map((carrier, index) => (
                  <div key={index} className="flex justify-between">
                    <span>载波{index + 1}:</span>
                    <span>n{carrier.band} ARFCN:{carrier.arfcn}</span>
                  </div>
                ))}
              </div>
            )}
